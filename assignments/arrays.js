...
let carModels = inventory.map(a => a.car_model).sort();
...
let oldCars = carYears.filter(function(item) {
    return item < 2000;
}).length;
console.log(oldCars); 
...
let BMWAndAudi = inventory.filter(function(el){
    return el.car_make === "BMW" || el.car_make === "Audi";
});
